...
print("Dispersion personnelle (Écart-type) :", std_etudiants)
...
print("Notes + moyenne individuelle (horizontal stack - hstack) :\n", notes_enrichies)

epreuves_ext = np.append(epreuves, "Moyenne")
print("Colonnes :", epreuves_ext)
moyenne_globale_finale = np.mean(moyennes_etudiants)

ligne_moyennes = np.append(moyennes_epreuves, [moyenne_globale_finale])
...
print("\nTableau avec écart-type personnel (sur les étudiants seulement) :\n", notes_info)
print("Colonnes finales :", epreuves_finales)

header = " | ".join(f"{col:>8s}" for col in epreuves_ext)
print(f"{' ':>20s} : {header}")
print("-" * (22 + len(header) + 5))
